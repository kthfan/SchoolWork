...
class FlexTable:
    ...
    def _compute_dim(self, elem):
        if isinstance(elem, [FlexTable, Table]):
    ...
    def put_text(self, row, col, item):
        self.table_entries["{}:{}".format(row, col)] = str(item)
    def get_tabel_dim(row=None, col=None):
        ...
        for i,r in enumerate(row):
            for j,c in enumerate(col):
                yx = "{}:{}".format(r, c)
                elem = self.sub_tablesa.get(yx) or self.table_entries.get(yx)
