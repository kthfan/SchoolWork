...
class PSOSolver:
    ...
    def fit(self, particles, max_iter=1000, tol=1e-6, stop_condition=None):
        if isinstance(particles, np.ndarray): particles = PSOParticles(particles, self.fitness_func)
        ...
        for current_iter in range(max_iter):
            ...
            if stop_condition is not None and stop_condition(particles):
                break
...
class PSOParticles:
    def __init__(self, particles, upper_bound=None, lower_bound=None, fitness_func=None):
    ...
    def update_solutions(self, solutions, fitness, update_velocities=True):
        ...
        # set best individual solutions and fitness
        should_update = fitness > self.best_fitness
